...
export class Action {
	...
	public checkAllPlayerTileActions(gameState: GameState) : GameState {
		return gameState.players.reduce((currentGameState: GameState, player: Player) => {
	...
	protected checkPlayerTileAction(player: Player, board: Board, score: number, outcome: string): { outcome: string, board: Board, score: number }  {
