...
const checkPlayerBoardCollision = (board: Board, playerTypes) => (player: Player): Player[] => {
    return (isCollision(board)(player)) ? splitPlayer(playerTypes)(player) : [player]
}
...
const isPlayerInTile = (player: Player) : boolean => (player.coords.offsetX === 0 && player.coords.offsetY === 0)
...
const playerFromItem = (playerTypes, player: Player) => (item: ISplitItem) : Player => {
